<template>
  <div class="hello">
    ...
    <img v-if="loaded"  :src="img">   <!--   <img v-if="loaded" :src="img"> 加了v-if="loaded"  反而无法显示 (子程序出错) -->
    <h2>X:{{ x }} Y:{{ y }}img:{{ img }}</h2>
    <h2>count:{{ count }}</h2>
    <h1>count1:{{ count1 }}</h1>
    <h1>double:{{ double }}</h1>
...
export default defineComponent({   //defineComponent函数，只是对setup函数进行封装，返回options的对象；defineComponent最重要的是：在TypeScript下，给予了组件 正确的参数类型推断 。
  ...
  setup() {
    ...
    //   console.l og(event)
    // })
    const {  result,  loaded, loading, img } = useURLloader<DogR>('https://dog.ceo/api/breeds/image/random');

       watch(result, () => {
         if (result.value) {
          console.log('result.result',result.value.message)

         }
       }),

    console.log("result: " + result.value + "  loading:" + loading.value + "  loaded:" + loaded)
    console.log("img::::: " + img.value);   console.log("img " + img);    console.log("result.value: " + result.value)
